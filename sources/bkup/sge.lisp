...
; -- to instantiate the decomp functions etc. in eval-once-mode
|#


; I need a function to convert an array back into an SDIF file.

(defmethod! sge-params (&key (markers '((0.1 0.2) (0.1 0.2) (0.1 0.2) (0.1 0.2))) (corpora t)  
...
(defmethod! sge-decomp ((snd sound) (maxatoms number) (srr number) (dict-path t) &key outpath)
            :icon 04
            :numouts 3
            ...
            (if (probe-file *sge-path*)
                (let* ((inpath (sound-path snd))
                       (filename (pathname-name inpath))
                       (name (or (and outpath (pathname-name outpath)) (format nil "~a_sge" filename)))
              ...
              (progn 
                (print "sgeDecomp not found... set in preferences?")
                nil
                ))
            )

(defmethod! get-sge-params ((self sdiffile) &optional mintime maxtime)
            :icon 04
            :numouts 8
            ...
            (let* ((sdiflist (flat (getsdifdata self 0 "XADS" "XSGM" nil nil nil mintime maxtime) 1))
              ...
              (values 
               ...
               (get-sge-paths self (om-round (third translist)) (om-round (fourth translist))) ;filepath (string)
              )))


(defmethod! get-sge-array ((self sdiffile) &optional mintime maxtime)
            :icon 04
            :numouts 1
            :outdoc '("the sge array")
            (let* ((thedata (multiple-value-list (get-sge-params self mintime maxtime)))
                   (thearray (make-instance 'sge-array 
...
(defmethod! get-sge-array-resamp ((self sdiffile) (numcols integer) &optional mintime maxtime)
            :icon 04
            :numouts 1
            :outdoc '("the re-sampled sge array")
            (let* ((thedata (multiple-value-list (get-sge-params self mintime maxtime)))
                   ...
                   (thearray (make-instance 'sge-array 
                                            ...
                                            :filepath (get-sge-paths self (om-round sampled-corpus-index) (om-round sampled-file-index))
                   )))
              thearray))



(defmethod objfromobjs ((self sdiffile) (type sge-array))
  (let* ((sdifdata (multiple-value-list (get-sge-params self)))
         (new (make-instance 'sge-array 
...
(defmethod! get-sge-paths ((self sdiffile) (corpusid list) (fileid list))
            (mapcar (lambda (corpora files)
                      (get-sge-paths self corpora files)) corpusid fileid)
            )

(defmethod! get-sge-paths ((self sdiffile) (corpusid number) (fileid number))
...
#|
(defun sge-amplitude (norm magnitude)
